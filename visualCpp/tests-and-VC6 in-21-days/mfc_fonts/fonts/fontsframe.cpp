...
{
	CRect rect;
	Create(NULL,_T("Fonts windows"),WS_OVERLAPPEDWINDOW|WS_VISIBLE,CRect(0,0,600,600));
	GetClientRect(&rect);
...
BOOL CFontsFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	cs.cx=600;
	cs.cy=800;
...
void CFontsFrame::OnPaint()
{
	...
	delete _pen;
	CBrush *oldBrush;
	CBrush *_brush=new CBrush(HS_BDIAGONAL,RGB(255,0,0));
	oldBrush=pDC.SelectObject(_brush);
	position+=20;
	pDC.Rectangle(20,position+50,200,position);
	pDC.SelectObject(oldBrush);
	delete _brush;
	_brush=new CBrush(HS_CROSS,RGB(255,0,0));
	pDC.SelectObject(_brush);
	position+=55;
	pDC.Rectangle(20,position+50,200,position);
	delete _brush;
	_brush=new CBrush(HS_DIAGCROSS,RGB(255,0,0));
	pDC.SelectObject(_brush);
	position+=55;
	pDC.Rectangle(20,position+50,200,position);
	delete _brush;
	_brush=new CBrush(HS_FDIAGONAL,RGB(255,0,0));
	pDC.SelectObject(_brush);
	position+=55;
	pDC.Rectangle(20,position+50,200,position);
	delete _brush;
	_brush=new CBrush(HS_VERTICAL,RGB(255,0,0));
	pDC.SelectObject(_brush);
	position+=55;
	pDC.Rectangle(20,position+50,200,position);
	delete _brush;
	_brush=new CBrush(HS_HORIZONTAL,RGB(255,0,0));
	pDC.SelectObject(_brush);
	position+=55;
	pDC.Rectangle(20,position+50,200,position);
	delete _brush;
