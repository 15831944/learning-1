...
BOOL CComboListDlg::OnInitDialog()
{
	...
	for(int i=0;i<3;i++) m_data[i]=i+1;
	m_combo.AddString("first");
	m_combo.AddString("second");
	m_combo.AddString("third");
	m_combo.SetItemDataPtr(0,&m_data[0]);
	m_combo.SetItemDataPtr(1,&m_data[1]);
	m_combo.SetItemDataPtr(2,&m_data[2]);
	return TRUE;  // return TRUE  unless you set the focus to a control
}

void CComboListDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CComboListDlg::OnSelendokcombo() 
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
	int sel=m_combo.GetCurSel();
	m_rez=*((int *)m_combo.GetItemDataPtr(sel));
	UpdateData(FALSE);
}
